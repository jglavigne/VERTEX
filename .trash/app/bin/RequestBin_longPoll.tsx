...
useEffect(() => {
  ...
  async function longPoll() {
    while (isActive) {
      try {
        // Récupérer le timestamp du log le plus récent en format ISO
        const allLogs = [...liveRequests, ...requests];
        let lastTimestamp;

        if (allLogs[0]?.created_at) {
          const date = new Date(allLogs[0].created_at);
          date.setMilliseconds(date.getMilliseconds() + 1); // ✅ +1ms
        ...
        if (!res.ok) throw new Error('Long poll failed');

        const data = await res.json();

        // Si nouveaux logs
        if (data.logs && data.logs.length > 0) {
          setLiveRequests(prev => [...data.logs, ...prev]); // Ajouter en haut
          setSelected(data.logs[0]); // Auto-sélectionner le premier
        }

      } catch (err: any) {
        if (err.name === 'AbortError') break;
        console.error('Long poll error:', err);
        await new Promise(r => setTimeout(r, 10000)); // Attendre 2s avant retry
      }
    }
  }
...
}, [isLiveMode, liveRequests, requests, clientId]);
    ...
    <div className={styles.leftPanel}>
      ...
      <div className={styles.listScroll}>
        {paginated.map((req) => (
          ...
          >
            <strong>{req.method}</strong> <small>{new Date(req.created_at).toLocaleString()}</small>
            <br />
            {req.path}
        ...
        <pre style={{ overflow: 'auto', maxHeight: '200px' }}>
          ...
          <div className={styles.detailsHeader}>
            <div className={styles.detailsTitle}>
              <em>{new Date(selected.created_at).toLocaleString()}</em> :{" "}
